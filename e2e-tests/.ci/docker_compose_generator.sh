...
# Note: For local development, environment variable ENABLED_DOCKER_SERVICES can be set to a subset of services
# for lightweight and faster start up time.
if [ $# -gt 0 ] && [ "$1" = "cypress" ]; then
  . .e2erc_cypress
elif [ $# -gt 0 ] && [ "$1" = "playwright" ]; then
  . .e2erc_playwright
else
  mme2e_log "Usage: ./docker_compose_generator.sh cypress|playwright"
  exit 1
fi
...
if [ -n "${ENABLED_DOCKER_SERVICES-}" ] && [ -n "$ENABLED_DOCKER_SERVICES" ]; then
  mme2e_log "ENABLED_DOCKER_SERVICES is not empty. Value: $ENABLED_DOCKER_SERVICES"
else
  # If not set, then remove the override file if it exists and exit
  mme2e_log "Environment variable ENABLED_DOCKER_SERVICES is empty or unset."
